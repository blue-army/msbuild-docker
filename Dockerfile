...
RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Workload.MSBuildTools --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) };

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Workload.NetCoreBuildTools --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Workload.WebBuildTools --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Workload.VCTools --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Component.VC.Tools.14.11 --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_buildtools.exe -ArgumentList '--add Microsoft.VisualStudio.Component.VC.140 --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.Component.Windows81SDK --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_community.exe -ArgumentList '--add Microsoft.VisualStudio.ComponentGroup.NativeDesktop.Win81 --quiet --nocache --wait --installPath C:\BuildTools'; \
	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

# RUN	$p = Start-Process -Wait -PassThru -FilePath c:\vs_buildtools.exe -ArgumentList '--add Microsoft.VisualStudio.Component.VC.Tools.x86.x64 --quiet --nocache --wait --installPath C:\BuildTools'; \
# 	if ($ret = $p.ExitCode) { c:\collect.exe; throw ('Install failed with exit code 0x{0:x}' -f $ret) }; 

#	\
#	rm "$env:TEMP\vs_buildtools.exe"

# $p = Start-Process -Wait -PassThru -FilePath $env:TEMP\vs_buildtools.exe -ArgumentList '
# --add Microsoft.VisualStudio.Workload.MSBuildTools 
# --add Microsoft.VisualStudio.Workload.NetCoreBuildTools 
# --add Microsoft.VisualStudio.Workload.VCTools 
# --add Microsoft.VisualStudio.Workload.WebBuildTools 
# --add Microsoft.VisualStudio.Component.VC.Tools.14.11 --quiet --nocache --wait --installPath C:\BuildTools'; \
